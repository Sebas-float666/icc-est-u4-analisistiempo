import java.util.Arrays;
import java.util.concurrent.Callable;

import controllers.SortMethods;
import models.Results;

public class App {
    public static void main(String[] args) throws Exception {
        
        int size=200;
        int[]arreglo=generate(size);
        int[]arreglo2=arreglo.clone();
        int[]arreglo3=Arrays.copyOf(arreglo, size);

        SortMethods metodos = new SortMethods();

        Callable<Void> funcionBubble = () -> {
            metodos.sortBubble(arreglo2);
            return null;
        };
        Results rBubble = new BenchMarking().medirTiempo(funcionBubble, "Burbuja", size);
        System.out.println(rBubble.getName() + " = "+ rBubble.getTime());

        
        Callable<Void> funcionBubbleAvan = () -> {
            metodos.sortBubbleAvan(arreglo3);
            return null;
        };
        Results rBubbleAvan = new BenchMarking().medirTiempo(funcionBubbleAvan, "Burbuja Avanzada", size);
        System.out.println(rBubbleAvan.getName() + " = "+ rBubbleAvan.getTime());  
    }
    
      
    
    public static int[] generate(int size) {
        int[] array = new int[size];
        for (int i = 0; i < size; i++) {
            int numero = (int) (Math.random() * 10000);
            array[i] = numero;
        }
        return array;
    }
}